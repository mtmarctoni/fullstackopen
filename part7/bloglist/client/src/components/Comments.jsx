...
const Comments = ({ isLogged }) => {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const newComment = await dispatch(
        commentBlog(blogId, content, loginUser.token)
      )
      e.target.comment.value = ''
      setComments([...comments, newComment])
      dispatch(
        createNotification(`You commented: ${newComment.content}`, 'success', 5)
      )
    } catch (err) {
      dispatch(createNotification('Comment failed', 'error', 5))
    }
  }
  ...
  return (
    <div>
      ...
      <form
        className={`${isLogged ? 'block' : 'hidden'} mb-4`}
      ...
      <ul className="list-disc pl-5">
        {comments.length !== 0 ? (
          comments.map((comment) => (
            <li key={comment.id} className="text-gray-700 mb-1">
              {comment.content} - {parseDate(comment.date)}
            </li>
          ))
        ) : (
